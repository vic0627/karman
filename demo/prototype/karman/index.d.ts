...
type ParamPosition = {
   ...
   * @description if true, url builder will use the same key of payloadDef and the received value to build the request url
   * @example
   * // assuming the base url is 'https://karman.com/' and the instance of Karman is '$k'
   * // ...
   * getSomthing: defineAPI({
...
declare class Karman extends Karman {
...
interface ApiOptions<ST, P, D, S, E> extends Hooks<ST, P, D, S, E>, UtilConfig, CacheConfig, RequestConfig<ST> {
  /**
   * endpoint of an API
   * @description if received value, the value would be place after the base url of current layer, and before all url parameters
   */
  endpoint?: string;
  /**
   * HTTP method
